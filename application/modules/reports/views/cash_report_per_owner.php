<div class="panel panel-success">
    ...
    <div class="panel-body">
        <div class="btn-group margin-bottom-10">
            ...
            <span><a href="<?php echo base_url_tr() ?>reports/per_owner_pdf?onwers_id=<?php echo $onwers_id; ?>&from_month=<?php echo $from_month; ?>&to_month=<?php echo $to_month; ?>" class="btn purple"><i class="fa fa-file-pdf-o"></i> PDF</a></span>
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="alert alert-success">
                    <form method="get" action="<?php echo base_url_tr('reports/per_owner'); ?>">
                        <div class="col-lg-3">
                            <div class="input">
                                <label>Owners</label>
                                <select name="onwers_id" id="apartment_id" class="form-control" required>
                                    <option value="">..Select..</option>
                                    <?php foreach ($owners as $value) {?>
                                    <option <?php echo $onwers_id && $onwers_id == $value["id"] ? 'selected="selected"' : null; ?> value="<?php echo $value["id"]; ?>"><?php echo $value["name"]." ".$value['family_name'] ; ?></option>
                                    <?php }?>
                                </select>
                            </div><!-- /input-group -->
                        </div><!-- /.col-lg-6 -->
                ...
                if($per_owner_inout_cash && count($per_owner_inout_cash) > 0) {
                    foreach($per_owner_inout_cash as $cash){
                        ...
                        <tr>
                            ...
                            <td><?php echo date('M Y', strtotime($cash->payment_date)); ?></td>
...
<script type="text/javascript">
    function costDetails(ownerId,ownerName){
        ...
        var total_amount=0;
        var total_revenue=0;
        $(".modal-title").html("Cost & Revenue Details ( "+ownerName+" )");
        $.ajax({
