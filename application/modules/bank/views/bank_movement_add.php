      ...
      <form class="item_add_form" id="calendar_view" action="<?php echo base_url(); ?>bank/bma_save" method="post">
        <div class="form-body col-xs-12">
          ...
          <div class="form-group">
            ...
            <input type="text" name="movement_amount" id="movement_amount" class="form-control"/>
          </div>
          ...
          <div class="form-group">
            ...
            <select name="movement_type" id="movement_type" class="form-control" required>
              ...
              <option value="2">Bank Out</option>
            </select>
          </div>
  ...
  function getUnpaidCost(id, user_type)
  {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        {
          if(user_type==3){
            if(data[i].revenue_amount>0){
              var html = '<option data-desc="'+data[i].description+'" data-amount="'+data[i].revenue_amount+'" value="'+data[i].id+'">Due date '+data[i].payment_date+' | '+data[i].revenue_amount+'</option>';
              $("#unpaid_cost").append(html);
            }
            if(data[i].oc_amount>0){
              var html = '<option data-desc="'+data[i].description+'" data-amount="'+data[i].oc_amount+'" value="'+data[i].id+'">Due date '+data[i].payment_date+' | '+data[i].oc_amount+'</option>';
              $("#unpaid_cost").append(html);
            }
          }
          if(user_type==2){
            var html = '<option data-desc="'+data[i].description+'" data-amount="'+data[i].revenue_amount+'" value="'+(user_type == "3"?data[i].gc_id:data[i].id)+'">'+'<strong>ID: </strong>'+(user_type == "3"?data[i].gc_id:data[i].id)+' - '+(user_type == "3"? '<strong>Description: </strong>'+data[i].description:'<strong>Revenue: </strong>'+data[i].revenue_amount)+(user_type == "3"?'<strong> - Amount: </strong>'+data[i].revenue_amount:"")+'</option>';
            $("#unpaid_cost").append(html);
          }
          if(user_type==1){
            var html = '<option data-desc="'+data[i].description+'" data-amount="'+data[i].revenue_amount+'" value="'+(user_type == "3"?data[i].gc_id:data[i].id)+'">'+'<strong>ID: </strong>'+(user_type == "3"?data[i].gc_id:data[i].id)+' - '+(user_type == "3"? '<strong>Description: </strong>'+data[i].description:'<strong>Revenue: </strong>'+data[i].revenue_amount)+(user_type == "3"?'<strong> - Amount: </strong>'+data[i].revenue_amount:"")+'</option>';
